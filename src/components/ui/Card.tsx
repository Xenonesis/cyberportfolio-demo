...
export const Card = ({
  ...
  const baseClasses =
    'bg-deep-navy-800 border border-deep-navy-600 rounded-xl overflow-hidden transition-all duration-300 hover:border-electric-cyan-500 hover:shadow-lg hover:shadow-electric-cyan-400/10 hover:scale-105 relative';
  ...
  const cardContent = (
    <>
      ...
      <div className='absolute top-2 left-2 z-10'>
        <div
          className={`w-3 h-3 rounded-full ${threatLevel === 'low' ? 'bg-neon-green-500' : threatLevel === 'medium' ? 'bg-yellow-500' : 'bg-red-500'} animate-pulse`}
        ></div>
      </div>
      ...
      <div className={contentClasses}>
        ...
        {title && (
          <h3 className='text-lg font-semibold text-white mb-2 flex items-center gap-2'>
            {title}
            <div
              className={`w-2 h-2 rounded-full ${threatLevel === 'low' ? 'bg-neon-green-500' : threatLevel === 'medium' ? 'bg-yellow-500' : 'bg-red-500'} animate-pulse`}
            ></div>
          </h3>
        )}
