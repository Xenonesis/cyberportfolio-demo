...
export const SecurityStatus = ({ 
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <div className={`rounded-full ${sizeClasses[size]} ${config.color} ${config.pulse ? 'animate-pulse' : ''}`}></div>
      {label && <span className="text-sm font-medium text-light-gray-300">{label}</span>}
      {!label && <span className="text-xs font-medium text-light-gray-400">{config.text}</span>}
    </div>
  );
};
...
export const ThreatLevel = ({ 
  ...
  return (
    <div className={`p-4 rounded-lg border-2 ${config.border} ${config.bg} ${className}`}>
      <div className="flex items-center gap-3">
        <span className="text-lg">{config.icon}</span>
        <div>
          {title && <h4 className={`font-semibold ${config.color}`}>{title}</h4>}
          {description && <p className="text-light-gray-400 text-sm">{description}</p>}
        </div>
      </div>
    </div>
  );
...
export const SecurityBadge = ({ 
  ...
  return (
    <div className={`${config.bg} ${config.border} rounded-md ${sizeClasses[size]} flex items-center gap-1 ${className} animate-shield-activate`}>
      <span>{config.icon}</span>
      <span className="font-medium text-electric-cyan-400">{label || config.text}</span>
    </div>
  );
};
    ...
    <div className={`w-full ${className}`}>
      {label && (
        <div className="flex justify-between items-center mb-2">
          ...
          {showPercentage && (
            <span className="text-electric-cyan-400 font-mono text-sm">{Math.round(percentage)}%</span>
          )}
        </div>
      )}
...
export const CircuitPattern = ({ 
  ...
  return (
    <div className={`relative ${className}`}>
      <div className={`absolute inset-0 ${sizeConfig[size]} ${intensityConfig[intensity]} circuit-pattern`}></div>
...
export const BinaryRain = ({ 
  ...
  return (
    <div className={`relative ${className}`}>
      <div className={`absolute inset-0 ${densityConfig[density]} ${speedConfig[speed]} binary-texture pointer-events-none`}></div>
      <div className="relative z-10">{children}</div>
    </div>
  );
};
...
export const DataFlow = ({ 
  ...
  return (
    <div className={`relative ${className}`}>
      <div className={`absolute inset-0 ${directionConfig[direction]} ${speedConfig[speed]} data-flow pointer-events-none`}></div>
      <div className="relative z-10">{children}</div>
    </div>
  );
};
