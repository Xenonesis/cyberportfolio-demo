...
export const KeyboardNavigation: React.FC<KeyboardNavigationProps> = ({
  ...
  // Focus traps
  const [focusTraps, setFocusTraps] = useState<Map<string, HTMLElement[]>>(new Map());
  ...
  const announceKeyboardNavigation = () => {
    ...
    liveRegion.textContent = 'Keyboard navigation is available. Press Ctrl+? for help.';
  };
  ...
  const setupSkipLinks = () => {
    ...
    skipMain.className = 'skip-nav bg-cyan-600 text-white px-4 py-2 rounded z-50 fixed top-4 left-4';
  ...
  const createFocusTrap = (id: string, elements?: HTMLElement[]): string => {
    const trapElements = elements || Array.from(
    ...
    <KeyboardNavigationContext.Provider value={keyboardContext}>
      ...
      >
        ...
        >
          ...
          {activeTrap && (
            <span>Focus trap active: {activeTrap}</span>
          )}
        </div>
      </div>
