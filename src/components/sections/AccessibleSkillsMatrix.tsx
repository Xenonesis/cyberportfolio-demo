...
export const AccessibleSkillsMatrix = ({ config = SKILLS_MATRIX_CONFIG, className = '' }: AccessibleSkillsMatrixProps) => {
    ...
    switch (event.key) {
      ...
      case 'ArrowRight':
        ...
        if (currentIndex < allCategories.length - 1) {
          ...
          setTimeout(() => {
            const nextButton = categoryButtonsRef.current[allCategories.indexOf(nextCategory)];
            nextButton?.focus();
          }, 0);
        }
        break;
      case 'ArrowLeft':
        ...
        if (currentIndex2 > 0) {
          ...
          setTimeout(() => {
            const prevButton = categoryButtonsRef.current[allCategories.indexOf(prevCategory)];
            prevButton?.focus();
          }, 0);
        }
        break;
      ...
      >
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          ...
          <button
            ...
            aria-label={showAllSkills ? 'Hide empty categories' : 'Show all categories including empty ones'}
          >
