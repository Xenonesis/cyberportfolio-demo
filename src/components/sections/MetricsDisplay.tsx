...
export const MetricsDisplay: React.FC<MetricsDisplayProps> = ({
  ...
  const aggregateMetrics = caseStudies.reduce(
    (acc, caseStudy) => {
  ...
  const averageImprovement =
    aggregateMetrics.totalImprovements > 0
      ? Math.round(
          aggregateMetrics.totalPercentageImprovement /
  ...
  const topCaseStudies = caseStudies
    ...
    .sort((a, b) => {
      const aImprovement =
        a.metrics?.reduce((sum, m) => sum + (m.isPositive ? 1 : 0), 0) || 0;
      const bImprovement =
        b.metrics?.reduce((sum, m) => sum + (m.isPositive ? 1 : 0), 0) || 0;
  ...
  return (
    ...
    >
                  ...
                  {caseStudy.metrics?.slice(0, 2).map(metric => (
                    ...
                    >
                      ...
                      <span
                        className={
                          metric.isPositive
                            ? 'text-neon-green-400 font-medium'
      ...
      >
        ...
        <div className='flex flex-wrap justify-center gap-2'>
          {Array.from(new Set(caseStudies.map(cs => cs.client.industry))).map(
            industry => (
              <span
                key={industry}
