...
export const TestimonialFilterBar: React.FC<TestimonialFilterBarProps> = ({
  ...
  const filterOptions = useMemo(() => {
    const industries = [
      ...new Set(testimonials.map(t => t.companyIndustry).filter(Boolean)),
    ] as string[];
    const companySizes = [
      ...new Set(testimonials.map(t => t.companySize).filter(Boolean)),
    ] as string[];
    const projectTypes = [
  ...
  const renderSecurityDomainFilters = () => (
    <div className='flex flex-wrap gap-2'>
      {securityDomainOptions.map(domain => {
        ...
        const count = testimonials.filter(t =>
          t.securityDomain?.includes(domain.id as any)
        ).length;
  ...
  const renderAdvancedFilters = () => (
    <div
      className={`space-y-4 transition-all duration-300 ${isAdvancedFiltersOpen ? 'block' : 'hidden'}`}
    >
      {/* Industry Filters */}
      <div>
  ...
  return (
    <div
      className={`bg-deep-navy-800 border border-deep-navy-600 rounded-xl p-6 ${className}`}
    >
      {/* Search and Sort Row */}
      <div className='flex flex-col lg:flex-row gap-4 lg:items-center justify-between mb-4'>
        ...
        <div className='flex items-center space-x-2'>
          ...
          <select
            ...
            onChange={e =>
              onSortChange(e.target.value as TestimonialFilters['sortBy'])
            }
          ...
          <div className='flex items-center space-x-2 text-sm'>
            ...
            <span className='text-electric-cyan-400 font-medium'>
              {filters.categories.length +
                filters.securityDomains.length +
...
export const QuickFilterChips: React.FC<{
  ...
  testimonials: EnhancedTestimonial[];
}> = ({ filters, onFilterChange, testimonials }) => {
  const topCategories = [
    {
      id: 'featured',
      label: 'Featured',
    ...
    },
    {
      ...
      count: testimonials.filter(t =>
        t.securityDomain?.includes('cloud-security')
      ).length,
    },
  ];
