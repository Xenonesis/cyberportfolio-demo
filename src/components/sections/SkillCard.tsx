...
export const SkillCard = ({
  ...
  const proficiencyLevel = proficiencyLevels.find(level => 
    skill.proficiency >= level.minPercentage && skill.proficiency <= level.maxPercentage
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-white text-lg mb-1">{skill.name}</h4>
            <Badge 
              variant="info" 
              size="sm"
              className="text-xs"
        ...
        {showProficiencyLevels && (
          <Badge 
            variant={proficiencyLevel.level === 'expert' ? 'success' : 'default'}
            size="sm"
        ...
        <div className="w-full bg-security-gray-600 rounded-full h-2 overflow-hidden">
          <motion.div
            className={`h-full rounded-full ${
              proficiencyLevel.level === 'expert' ? 'bg-neon-green-500' :
              proficiencyLevel.level === 'advanced' ? 'bg-electric-cyan-500' :
              proficiencyLevel.level === 'intermediate' ? 'bg-light-gray-400' :
              proficiencyLevel.level === 'beginner' ? 'bg-security-gray-500' :
              'bg-deep-navy-500'
            }`}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {skill.certificationLinks && skill.certificationLinks.length > 0 && showCertifications && (
          <Badge variant="default" size="sm" className="text-xs">
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300 bg-black/50 rounded-xl p-4">
        <div className="text-white text-sm">
          <p><strong>Experience:</strong> {formatExperience(skill.yearsOfExperience)}</p>
          {skill.projectExamples && skill.projectExamples.length > 0 && (
            <p><strong>Projects:</strong> {skill.projectExamples.length} case studies</p>
          )}
        </div>
      </div>
    </motion.div>
