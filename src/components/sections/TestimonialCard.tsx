...
import { Star, Shield, CheckCircle, AlertCircle, Users, Award } from 'lucide-react';
...
export const TestimonialCard: React.FC<TestimonialCardProps> = ({
  ...
  const getCompanySizeBadge = (size?: string) => {
    const badges = {
      ...
      'mid-market': 'bg-purple-600/20 text-purple-400 border border-purple-600/30',
      enterprise: 'bg-red-600/20 text-red-400 border border-red-600/30',
    };
    return badges[size as keyof typeof badges] || 'bg-gray-600/20 text-gray-400 border border-gray-600/30';
  };
  ...
  const formatSecurityMetrics = () => {
    ...
    if (testimonial.securityMetrics?.vulnerabilitiesFound !== undefined) {
      metrics.push(`${testimonial.securityMetrics.vulnerabilitiesFound} vulnerabilities found`);
    }
    
    if (testimonial.securityMetrics?.securityScoreImprovement) {
      metrics.push(`${testimonial.securityMetrics.securityScoreImprovement}% security improvement`);
    }
    
    if (testimonial.securityMetrics?.incidentResponseTime) {
      metrics.push(`Response time: ${testimonial.securityMetrics.incidentResponseTime}`);
    }
    
    if (testimonial.securityMetrics?.complianceAchieved?.length) {
      metrics.push(`Compliance: ${testimonial.securityMetrics.complianceAchieved.join(', ')}`);
    }
      ...
      <div className="p-6 relative">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            {testimonial.companySize && (
              <span className={`px-2 py-1 text-xs rounded-md ${getCompanySizeBadge(testimonial.companySize)}`}>
                {testimonial.companySize}
              </span>
            )}
          </div>
        ...
        <div className="flex items-start space-x-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-electric-cyan-400 text-sm font-medium mb-1">{testimonial.role}</p>
            <p className="text-gray-400 text-sm">{testimonial.company}</p>
            {testimonial.companyIndustry && (
              <p className="text-gray-500 text-xs mt-1">{testimonial.companyIndustry}</p>
            )}
          </div>
        </div>

        {/* Security domains/tags */}
        {testimonial.securityDomain && testimonial.securityDomain.length > 0 && (
      ...
      <div className="relative p-8">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div className="flex-1">
            ...
            {testimonial.companyIndustry && (
              <p className="text-gray-400 text-sm">{testimonial.companyIndustry}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-lg font-semibold text-white ml-2">{testimonial.rating}/5</span>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {testimonial.securityMetrics.vulnerabilitiesFound !== undefined && (
              <div className="text-center">
                <div className="text-2xl font-bold text-neon-green-400">{testimonial.securityMetrics.vulnerabilitiesFound}</div>
                <div className="text-xs text-gray-400">Vulnerabilities Found</div>
              </div>
            )}
            {testimonial.securityMetrics.securityScoreImprovement && (
              <div className="text-center">
                <div className="text-2xl font-bold text-electric-cyan-400">{testimonial.securityMetrics.securityScoreImprovement}%</div>
                <div className="text-xs text-gray-400">Security Improvement</div>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-sm text-gray-400">
          ...
          {testimonial.projectDuration && (
            <span className="text-electric-cyan-400 font-medium">{testimonial.projectDuration}</span>
          )}
        </div>
      </div>
    </motion.div>
