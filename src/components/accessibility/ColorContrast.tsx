...
export const ColorContrast: React.FC<ColorContrastProps> = ({
  ...
  const { currentContrast, adjustContrast, addError, addSecurityAlert } =
    useAccessibility();
    ...
    switch (level) {
      case 'normal':
        root.style.setProperty(
          ...
          wcagRequirements.normal.toString()
        );
        root.style.setProperty(
          '--contrast-ratio-large',
          wcagRequirements.large.toString()
        );
        break;
      case 'enhanced':
      ...
      case 'high':
        root.style.setProperty(
          '--contrast-ratio-normal',
          wcagRequirements.enhanced.toString()
        );
        root.style.setProperty(
          '--contrast-ratio-large',
          wcagRequirements.large.toString()
  ...
  const reportContrastIssues = (minContrast: number) => {
    const severity =
      minContrast < 3.0 ? 'critical' : minContrast < 4.5 ? 'warning' : 'info';
  ...
  const checkContrastLevels = () => {
    try {
      ...
      textElements.forEach(element => {
        ...
        const isLargeText =
          fontSize >= 18 || (fontSize >= 14 && getFontWeight(element) >= 700);
        const requiredContrast = isLargeText
          ? wcagRequirements.large
          : wcagRequirements.normal;
...
const ColorContrastContext = React.createContext<
  ColorContrastContextType | undefined
>(undefined);
...
export const useColorContrast = (): ColorContrastContextType => {
  ...
  if (context === undefined) {
    throw new Error(
      'useColorContrast must be used within a ColorContrast component'
    );
  }
  return context;
};
